...
# Corpus path: $OMEGA_SIGMA_CORPUS, else a sibling checkout if present. Corpora are external data.
SIGMA = Path(os.environ.get("OMEGA_SIGMA_CORPUS")) if os.environ.get("OMEGA_SIGMA_CORPUS") else \
    Path(__file__).resolve().parents[1] / "semantic-cyber/data/sigma-rules"

rules, ir, report, corpus = [], [], None, None
if SIGMA.is_dir():
    ...
    print(f"omega REPL — no corpus at {SIGMA}. Set OMEGA_SIGMA_CORPUS, or call load(<dir>) yourself.")
    print("  in scope: load, load_ir, to_ir, Corpus, Atom, Block, CompiledRule")
